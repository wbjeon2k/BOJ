...
//1. dist(i,j)가 i,j칸에 차게 되는 물의 높이를 뜻하는게 맞음. 처음에 pq에 0,0으로 넣고 시작하면 자연스럽게 맵 테두리는 전부 높이 0으로 설정됨.
//2. pq를 써서 높이가 낮은 순서대로 갱신하는 이유는 최소가 아닌 셀을 먼저 뽑아서 주변을 갱신해버리면 나중에 진짜 최소인 다른 셀로 갱신하려 할 때 모순될 수 있음
